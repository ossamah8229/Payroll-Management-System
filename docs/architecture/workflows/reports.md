...
frontend (Checkpoint 1B) is implemented in code (`reports-overtime-report-page.tsx` and siblings) but
...
## 20. Salary Release Report — Checkpoint 1A (Backend Foundation)

Backend, shared contracts, and backend tests only — no frontend page, no detail endpoint, no
schema/migration change. Built against the approved Checkpoint 0 architecture review's own frozen
decisions (`docs/PROJECT_PROGRESS.md`'s "Phase 7 Reports — Salary Release Report" entry).

### 20.1 Frozen decisions this checkpoint is built against

1. **Business purpose**: a single-cycle operational release-reconciliation report — which employees
   were released this Cycle, which remain Pending, which are Held, which resolved No Pay Due, which
   are Recovery Due, what the original salary amount released was, what remains pending for
   unreleased payable entries, which rows carry correction/balance-salary context, and when a
   released salary completed release. Deliberately not a cross-cycle employee release history
   (Employee Payroll History's own job), not a full audit report, not a Corrections ledger, not a
   Statements replacement, and not a payment-channel analytics report.
2. **Report grain**: one row = one `PayrollEntry`. `PayrollUnitRelease` is implementation/audit
   context for the release workflow — never this report's own list grain. No per-Unit release-event
   row, no per-Site aggregate-only row.
3. **Cycle**: exactly one, required. No range, no optional Cycle, no date-range substitute — the
   same canonical single-cycle shape Project Site Payroll Report/Deduction Report/Overtime Report
   already established.
4. **Permission — the first Reports-module report approved for an OR gate**: `reports:view` OR
   `payroll:view` (`requirePermission([PERMISSIONS.REPORTS_VIEW, PERMISSIONS.PAYROLL_VIEW])`,
   `salary-release-report`'s own `SALARY_RELEASE_REPORT_PERMISSION` in `reports.routes.ts`), reusing
   the exact any-of-permission array form `payroll-entry.routes.ts`'s own `VIEW_PERMISSIONS` already
   established (`requirePermission` has natively supported an array/any-of form since before this
   checkpoint — no custom middleware was written). Finance holds `payroll:view`/`payroll:release` but
   not `reports:view`; Payroll Staff holds `reports:view` but not `payroll:view`; gating on
   `reports:view` alone would have excluded Finance — the role that actually executes releases — from
   a report whose whole subject is release reconciliation. `payroll:release` alone never implies
   access (a dedicated test proves a `payroll:release`-only agent is rejected with 403). Master/global
   bypass is handled entirely by the existing authorization architecture, unaffected by this route's
   own gate.
5. **Site authorization**: always `PayrollEntry.siteId`, never `Employee.siteId` — reusing
   `assertSiteAccess`/`getAccessibleSiteIds` exactly as every other report in this module.
6. **Release-status model**: reuses the existing generic `derivePayrollEntryRowStatus`/
   `payrollEntryRowStatusWhereClause` (`payroll-entry-row-status.ts`) as-is — this report's fourth/
   fifth consumer, well past this project's own "extract at the third consumer" threshold. No second
   Released/Unreleased status enum, no independent hold logic, no frontend-oriented duplication.
   Release domain semantics (per-Unit release via `PayrollUnitRelease`, held-entry exclusion from the
   ordinary sweep, `NO_PAY_DUE`/`RECOVERY_DUE` as real payout outcomes, irreversibility, the deferred/
   unimplemented `PayrollUnitReadiness`) are read, never mutated or reinterpreted — this report is
   strictly read-only.
7. **Original Released Amount — financial-correctness-critical, proven immutable before
   implementation began**: `calcNet` applied to a `RELEASED` entry's own immutable stored columns.
   Traced directly against the actual Corrections/Balance-Adjustment write paths, not assumed from
   documentation: `corrections.service.ts`'s `approveCorrectionRequest` never issues a
   `tx.payrollEntry.update` of any kind — it only ever writes `Correction`/`BalanceAdjustment`/
   `CorrectionRequest` rows, and requires the entry already be `released = true` before a correction
   may even be proposed against it. `correctionBalancePayable`/`correctionBalanceRecovery` — the two
   calc-input columns a correction's own settlement could plausibly touch — are written exclusively
   by `recomputeAndPersistEntryAggregates` (`corrections.materialization.service.ts`), whose sole
   caller (`materializeOneAdjustmentLocked`) hard-rejects any target entry with `released = true` via
   ...
   and no release-time consumption path (`consumeMaterializationsForReleasedEntries`,
   `settleAdvancesForReleasedEntries`) writes back to `PayrollEntry`'s own calc-input columns. This is
   the identical property `employee-payroll-history.test.ts`'s own "the original released Net Salary
   on the main row is never replaced by a correction-replayed figure" test already proves for an
   equivalent figure — this checkpoint's own test suite adds the same proof under its own name (§20.6)
   rather than assuming it. For a non-`RELEASED` row, `originalReleasedAmount` is `null` — never a
   pending calculated net salary mislabeled as an amount that was released.
8. **Correction/balance-salary boundary**: Correction Count, Correction Balance Payable, Correction
   Balance Recovery only, as separate columns/context — never folded into a rewritten "original
   release." No correction reason, before/after value, actor, full settlement history, or live
   `BalanceAdjustment.remainingAmount`. No detail endpoint in V1 — Employee Payroll History already
   owns the detailed correction drill-down.
9. **Release actor**: Released At is included; Released By is deliberately excluded from the V1
   list/export/print — `PayrollEntry.releasedBy` may represent whichever Unit-release action happened
   to complete the final touched Unit for a multi-Unit entry, not a semantically reliable "person who
   released this employee's salary." No email, user id, or audit actor of any kind is ever exposed.
10. **Payment Method/channel**: excluded from V1. The schema stores no canonical release/payment
    channel — `bankId IS NULL` is an inference about banking configuration, not authoritative release
    metadata, and is never presented as one. No payment-method filter, no channel totals.
11. **Filters**: exactly `cycleId` (required), `siteIds` (multi-select), `unitId` (existing
    single-Site convention), `rowStatus`, `hasCorrection` (tri-state). No Employee search,
    Designation, release-date range, Released By, payment method, a separate Released/Held toggle
    (Row Status already covers the release-state question), or Current Roster Status.
12. **Columns**: `payrollEntryId`/`employeeId`/`employeeCode`/`employeeName`/`siteId`/`siteName`/
    `primaryUnit`/`additionalUnitCount`/`designation`/`rowStatus`/`originalReleasedAmount`/
    `correctionBalancePayable`/`correctionBalanceRecovery`/`releasedAt`/`correctionCount`. No
    Released By, CNIC, account number, IBAN, bank details, Payment Method, correction reason, audit
    actor, or `BalanceAdjustment.remainingAmount`.
13. **Totals**: `matchingCount`, the five status-breakdown counts, `correctedEntryCount` (always-
    exact DB aggregates, independent of `totalsComputed`), and `releasedAmount`/
    `pendingReleaseAmount`/`correctionBalancePayableTotal`/`correctionBalanceRecoveryTotal` (the
    bounded fetch-and-`calcNet`/`sumMoney` strategy every sibling report's own monetary totals
    already use, gated by `SALARY_RELEASE_REPORT_EXPORT_MAX_ROWS = 20,000`). `releasedAmount` sums
    only genuinely `RELEASED` rows' net salary (`netSalary > 0` defensively filtered, mirroring
    Payroll Summary's/Bank Sheet's own guard); `pendingReleaseAmount` sums only genuinely `PENDING`
    rows' net salary — the two are never combined into one "Total Released" figure.
    `correctionBalancePayableTotal`/`correctionBalanceRecoveryTotal` sum the raw stored columns
    across every matching row regardless of status, mirroring Payroll Summary's own
    `balancePayableIncluded`/`recoveryDeducted` convention.
14. **Sorting**: DB-native only — `employeeCode`/`employeeName`/`site`/`rowStatus`/`releasedAt` (a
    genuine, plain stored `PayrollEntry` column — the one field in this report that needed no bounded
    in-memory sort exception, unlike every sibling report's own `netSalary`).
    `originalReleasedAmount`/`correctionBalancePayable`/`correctionBalanceRecovery` are deliberately
    not sortable in V1. Every sort ends with a deterministic `PayrollEntry.id ASC` tie-break.
15. **No detail page/endpoint in V1.** Every field the frontend will ever need lives on the list row
    itself.
16. **Export**: CSV/XLSX, complete filtered dataset, the same 20,000-row preflight-`COUNT`/
    structured-413 pattern every sibling report already uses. No backend PDF.
17. **No schema/migration change expected** — proven with `EXPLAIN ANALYZE` in this checkpoint rather
    than assumed (§20.5).

### 20.2 Shared contract

`shared/src/schemas/salary-release-report.ts` — list/export query schemas and the full response
contract (`SalaryReleaseReportRow`/`Totals`/`ListResponse`/`ExportLimitError`), following the
identical shared-schema-validation convention every sibling report already established. Re-declares
its own 5-state `SalaryReleaseReportRowStatus` union under its own name (independently, per this
module's own "no confusing cross-report type reference in a public DTO" convention) rather than
importing a sibling report's own type.

### 20.3 Backend service and routes

`backend/src/modules/reports/salary-release-report.service.ts` — structurally mirrors
`project-site-payroll.service.ts`/`deduction-report.service.ts` (`resolveSiteIdFilter`/
...
verbatim (§20.1 item 7), so the financial-correctness reasoning travels with the code.

`GET /api/v1/reports/salary-release` and `GET /api/v1/reports/salary-release/export`, mounted on the
existing `reportsRouter`, both gated by the new `SALARY_RELEASE_REPORT_PERMISSION`
(`requirePermission([PERMISSIONS.REPORTS_VIEW, PERMISSIONS.PAYROLL_VIEW])`). No `/:id` route (frozen
decision 15). Both routes are audited (`report.viewed`/`report.exported`, `metadata.reportType:
'salary_release_report'`), following the same "audit the view, not just the export" convention every
other report already establishes. The export route preflight-`COUNT`s before generating any CSV/XLSX
buffer, returning a structured `{ code: 'EXPORT_ROW_LIMIT_EXCEEDED', matchingCount, maxRows, message }`
(HTTP 413) over the ceiling — never a silently truncated file, never `page`/`pageSize` accepted at all.

### 20.4 Table columns and totals

Row: Employee Code, Employee Name, Project Site, Primary Unit (+N more), Designation, Row Status,
Original Released Amount, Correction Balance Payable, Correction Balance Recovery, Released Date,
Correction Count. No Released By, no CNIC, no banking field, no Payment Method, no correction reason,
no audit actor — verified by a full-response recursive sensitive-key sweep in the backend test suite.

Totals: `matchingCount`, the five status-breakdown counts, `correctedEntryCount`, `releasedAmount`,
`pendingReleaseAmount`, `correctionBalancePayableTotal`, `correctionBalanceRecoveryTotal`, computed
over the **complete filtered dataset**, never the current page. No per-Unit total anywhere in the
response.

### 20.5 Performance evidence (measured, not assumed)

Seeded 10 sites × 1,000 employees × 3 cycles = 30,000 real `PayrollEntry` rows, the target cycle's
own 10,000-employee population deliberately distributed across every release-state bucket (40%
RELEASED/30% PENDING/10% HELD/10% NO_PAY_DUE/10% RECOVERY_DUE — mutually exclusive per the schema's
own `PayrollEntry_payoutOutcome_released_check` CHECK constraint), plus a multi-Unit subset and a
corrected-entry subset — a committed, repeatable Jest suite
(`backend/tests/salary-release-report-performance.test.ts`, 12 tests) covering the 8 representative
shapes named in the authorizing instruction, with real `EXPLAIN (ANALYZE, BUFFERS)`:

| Query | Plan | Execution time |
|---|---|---|
| List, one cycle only, all sites (10,000 of 10,000 matching), `ORDER BY employee.name` | `Index Scan` on `PayrollEntry_cycleId_hold_released_payoutOutcome_idx`, nested-loop into `Employee_pkey` | ~44ms |
| List, one cycle + one site (1,000 of 10,000 matching) | `Index Scan` on `PayrollEntry_siteId_cycleId_idx` | ~5ms |
| List, one cycle + one site + one unit (work-line join) | — | ~1.3s |
| `rowStatus=RELEASED` (~40% selectivity) | `Index Scan` on `PayrollEntry_cycleId_siteId_idx`, `Filter: released` applied post-scan | ~5ms |
| `rowStatus=HELD` (~10% selectivity) | — | ~0.1s |
| `rowStatus=PENDING` (~30% selectivity) | `Index Scan` on `PayrollEntry_cycleId_siteId_idx`, `Filter` applied post-scan | ~5ms |
| Sort by `releasedAt` desc (a plain stored column, no bounded fallback) | `Index Scan` on `PayrollEntry_cycleId_hold_released_payoutOutcome_idx`, in-DB `Sort` | ~9ms |
| Full HTTP request: list page + totals over 10,000 matching rows, one cycle, unfiltered | — | ~1.0s |
| Export: 1,000 matching rows (one cycle + one site) to CSV | — | ~0.2s |
| Export: 10,000 matching rows (one cycle, unfiltered) to CSV | — | ~1.7s |

**No `Seq Scan` on `PayrollEntry` occurs in any measured query shape** — the existing composite
`PayrollEntry_cycleId_hold_released_payoutOutcome_idx`, `PayrollEntry_siteId_cycleId_idx`, and
`PayrollEntry_cycleId_siteId_idx` indexes already cover every filtered/sorted shape this report
needs, including `releasedAt` sorting (no dedicated index on `releasedAt` alone was needed). **No
migration is proposed or recommended by this checkpoint.**

This sandbox's single-node local Postgres (via `embedded-postgres`) is not a production-scale cloud
database — these numbers are evidence of correct query-plan behavior and rough order of magnitude,
not a production SLA guarantee, the same caveat every prior report's own performance evidence states.

**19,999/20,000/20,001-row boundary, proven at real volume**
(`backend/tests/salary-release-report-boundary.test.ts`, 6 tests, mirroring
`deduction-report-boundary.test.ts`'s own efficient bucket strategy — one cycle, 20,001 real
`PayrollEntry` rows split 19,999/1/1 across three sites): `totalsComputed` is `true` at 19,999 and
exactly at 20,000, `false` at 20,001; export succeeds (200, full row count) at 19,999 and exactly at
20,000, and is rejected with a structured 413 at 20,001, before any row is fetched.

### 20.6 Tests

`backend/tests/salary-release-report.test.ts` (51 tests) — authorization (401; all six OR-gate
...
alone rejected; Master Admin global access; site scoping proven under both admitting permissions; an
explicit inaccessible-`siteIds` filter rejected with 403; a genuine historical-transfer scenario
proving `PayrollEntry.siteId`-based authorization), the cycle requirement (missing/malformed/
nonexistent `cycleId`, a range parameter proven to have no effect), row grain and columns (every
required column present, sensitive-field exclusion including Released By/Payment Method via both
spot checks and a recursive sweep), row-status derivation (all five states, `rowStatus` filter
parity), **release semantics** (a genuine two-work-line entry staying non-`RELEASED` until both
touched Units release; held-entry exclusion from the ordinary sweep; a real Late/Straggler Sweep
releasing a newly-eligible entry created after its Unit already released, while leaving the
already-resolved and still-held entries untouched; `NO_PAY_DUE`/`RECOVERY_DUE` via real release
sweeps, never carrying an amount), **the Original Released Amount critical regression** (release an
entry for real through the actual release endpoint, capture its amount, perform a real post-release
Correction, verify the stored `PayrollEntry` row is byte-for-byte unchanged — `version`/`updatedAt`
included — and the report's own amount is unchanged; a second test proving a materialized settlement
landing on a *later* cycle's entry never contaminates the *originating* released entry's own amount
or correction-balance fields), filtering (Site, Unit scoped to Site with a 400 mismatch, Has
Correction tri-state, unsupported filters proven inert including Released By/payment method/a
separate Released toggle), sorting (every approved field, the rejected `originalReleasedAmount`/
`cycle` sort fields, a deterministic tie-break proof across equally-sorted rows), pagination
(database-level, zero page overlap), totals (status counts summing to `matchingCount`,
`releasedAmount` proven to sum only `RELEASED` rows never Held/Pending/NO_PAY_DUE/RECOVERY_DUE or a
later balance-salary amount, `correctionBalancePayableTotal`/`RecoveryTotal` summing across every
matching row regardless of status, no per-Unit total anywhere), export (CSV/XLSX exact header order
and field-by-field parity against the list endpoint, a recursive sensitive-field sweep, filter/sort
parity, pagination ignored, permission enforced before any export work), and query discipline (a
`jest.spyOn` proof that correction counts are one batched query regardless of row count, a multi-unit
entry never multiplying row count, and — added by §20.8's independent review — the same proof
repeated with an explicit `unitId` filter matched on the entry's non-primary Unit). All 52 passing
(52 tests as of the independent hostile review pass below — originally 51; §20.8 added 1 regression
test, no existing test changed).

`backend/tests/salary-release-report-performance.test.ts` (12 tests, §20.5's own evidence) —
committed and repeatable, not an uncommitted smoke test.

`backend/tests/salary-release-report-boundary.test.ts` (6 tests, §20.5's own evidence) — the exact
19,999/20,000/20,001 boundary proven at real seeded volume.

**69/69 new backend tests passing at the time this checkpoint was first written (now 70/70 — see
§20.8).** Combined regression run with every sibling report (`payroll-entry-row-status.test.ts`,
...
### 20.7 What Checkpoint 1A did NOT build

Per its own explicit scope boundary: no frontend route, page, filter UI, print, or CSV/XLSX download
button — the backend/export endpoints exist and are fully functional over HTTP, but nothing in the
frontend calls them yet. No detail endpoint (frozen decision 15). No per-Unit financial total of any
kind. No schema or migration change (frozen decision 17, proven in §20.5). Dashboard and
Variance/Month-on-Month Report remain untouched and **Not Started**.

**Checkpoint 1A is backend-only and awaits explicit authorization before Checkpoint 1B (frontend)
begins.** No other report or Dashboard work was started. This checkpoint was not committed or pushed.

### 20.8 Independent hostile review (2026-08-11) — APPROVE WITH FIXES

An adversarial, independent re-review of Checkpoint 1A above, per explicit instruction. Scope:
repository/diff integrity re-verified (`main == origin/main == 9a9dee1`, working tree contains only
Salary Release Report's own shared contract/service/routes/tests/docs — no frontend, no Dashboard, no
Prisma schema/migration, `git diff --check` clean). Production code was traced directly, not trusted
from its own doc comments: every `payrollEntry.update`/`updateMany` call site in the entire backend
(`payroll-release.service.ts`, `corrections.repository.ts`'s `updatePayrollEntryCorrectionAggregates`,
`advances.service.ts`, `eobi-sync.service.ts`, `payroll-entry.service.ts`'s single/bulk/work-line
paths) was individually inspected and confirmed either gated on `released = false` or occurring
exactly once, atomically, as part of the release write itself — independently confirming the Original
Released Amount immutability claim (§20.1 item 7) from the architecture side, not merely from the
test suite. The OR-permission gate (`requirePermission`'s native any-of array support) and site
authorization (`assertSiteAccess`/`getAccessibleSiteIds`, no `globalPermission` override — confirming
the OR gate cannot widen Site access) were both re-traced against their shared implementations.
Multi-Unit/Late-Entry release semantics, release-status financial semantics, sensitive-value (not just
key-name) exclusion, the 19,999/20,000/20,001 boundary, CSV/XLSX/list parity, sort/pagination
stability, and query discipline were all re-verified against real seeded-Postgres test runs (51/6/12
tests, all green) rather than assumed from the source.

**No production code defect found.** Three test-coverage gaps were closed with new regression tests
(no existing test weakened): (1) `salary-release-report.test.ts`'s own multi-status test asserted
`originalReleasedAmount === null` for HELD/PENDING but never asserted `releasedAt === null` for HELD
specifically; (2) no test filtered a multi-Unit `PayrollEntry` by its *second* (non-primary) Unit and
proved exactly one row — the no-fan-out property of Prisma's `workLines: { some }` filter had only
been proven for the *unfiltered* multi-Unit case; (3) `payroll-entry-row-status-regression.test.ts`
(the M4 cross-consumer row-status proof) had not been extended to include Salary Release Report as a
fourth consumer, despite it being an equally-generic consumer of `derivePayrollEntryRowStatus`/
`payrollEntryRowStatusWhereClause`. Test count: 70 (52 + 6 + 12), up from 69 — plus 2 more assertions
inside the two existing `payroll-entry-row-status-regression.test.ts` tests (still 2 `it` blocks, now
covering four reports instead of three).

Investigated and found not applicable to this checkpoint: no "ScrollInvestigation Site" or contaminated
test-data incident, and no Puppeteer-timeout regression, is disclosed anywhere in this checkpoint's own
`SESSION_HANDOFF.md`/`PROJECT_PROGRESS.md` history, nor does either term appear anywhere in the
repository — a direct `ps aux` check found a single idle `payroll_dev` Postgres process and no stray
Puppeteer/Chrome-headless processes. This class of incident belongs to a different checkpoint's own
history, not this one; nothing here needed cleanup.

The performance suite's own `EXPLAIN (ANALYZE, BUFFERS)` plans were independently re-run at the full
30,000-row fixture: every measured shape still uses `Index Scan using "PayrollEntry_cycleId_idx"` (or
the composite/site-scoped equivalent), zero `Seq Scan`, `releasedAt` sorting confirmed genuinely
DB-native. No migration is recommended, consistent with §20.5's own conclusion. (The suite does not run
a standalone `ANALYZE "PayrollEntry"` before its `EXPLAIN ANALYZE` calls — checked against every
sibling report's own performance suite and found to be the majority convention already in this module,
`overtime-report-performance.test.ts` being the one outlier that does; not a defect introduced by this
checkpoint.)

**Verdict: APPROVE WITH FIXES** (the fixes being the three added regression tests; no production
defect existed). Verification after remediation: `salary-release-report.test.ts` (52/52),
`salary-release-report-boundary.test.ts` (6/6, real 19,999/20,000/20,001-row volume),
...
combined with every sibling report's own suite — **368/368 passing.** `shared`/`backend`
`npx tsc --noEmit` clean, `npm run lint` clean (0 errors, 10 pre-existing warnings confined to
unrelated `backend/scripts/*.ts` files), `npm run build` clean across `shared`/`backend`/`frontend`,
`git diff --check` clean. No commit, push, or deployment occurred during this review. See
`docs/SESSION_HANDOFF.md` Addendum 41 for the full evidence log.

**Salary Release Report Checkpoint 1A is backend-only and awaits explicit authorization before
Checkpoint 1B (frontend) begins.** No other report or Dashboard work was started or touched during
this review.

## 20.9 Checkpoint 1B — Frontend, OR Permission Gate, Browser Print, and E2E (2026-08-11)

Frontend-only, over the frozen Checkpoint 1A backend above (`main == origin/main ==
6b5fdca33c1dc6d5d543c8811a41501f0234ccc5` verified before any change) — no backend, shared-contract,
or database change in this checkpoint. **The first Reports-module report gated on an OR permission**
(`reports:view` OR `payroll:view`), carried through the frontend at every layer: the route guard, the
catalogue card, and the page's own internal fallback gate. No detail page was added (frozen decision)
and no Variance/Month-on-Month Report or Dashboard work was started.

**The OR gate required widening two places beyond Salary Release Report's own route/card — a genuine
architectural gap this checkpoint's own hostile self-check (§23 of the authorizing instruction)
predicted and caught.** `RequirePermission` (`components/layout/require-permission.tsx`) already
accepted `permission: PermissionKey[]` with any-of semantics — no new guard component was needed for
the report's own route. But the shared `/reports` catalogue route and `ReportsPage`'s own internal
`canView` check were both `reports:view`-only, so a `payroll:view`-only Finance user could not reach
the catalogue shell at all, regardless of Salary Release Report's own card being correctly gated —
failing the checkpoint's own required scenario ("payroll:view-only user: catalogue access"). Fixed by
widening both to `[PERMISSIONS.REPORTS_VIEW, PERMISSIONS.PAYROLL_VIEW]` (`App.tsx`'s `/reports` route,
`reports-page.tsx`'s `canView`) and, since every *other* catalogue entry's own `requiredPermission` had
previously been implicitly `undefined` (meaning "no requirement beyond whatever gate got the user onto
this page" — safe only while that page-level gate was `reports:view` alone), redefining an absent
`requiredPermission` to mean "the ordinary `reports:view` requirement every report before this one
already assumed" (`isCatalogueEntryVisible`'s own explicit default) — so a payroll:view-only user sees
the catalogue shell and exactly the one card they're actually authorized to open, never every other
report's own card as a live-looking link that still 403s on its own unchanged, still-`reports:view`-
only route. Proven directly: `reports-page.test.tsx` gained tests for a payroll:view-only user seeing
the Salary Release Report card and *none* of the six reports:view-only sibling cards, plus the
page-level admit/deny boundary itself.

**Fixed during independent hostile review (2026-08-11) — the Reports *sidebar* nav item is now widened
to the same OR gate.** Checkpoint 1B's original submission left `nav-config.ts`'s "Reports" entry
pinned to `reports:view` alone, reasoning that sidebar visibility is a UX convenience only, never the
actual security boundary, and that a pre-existing `nav-config.test.ts` case pinned the old behavior.
That left a payroll:view-only Finance user able to reach the Salary Release Report catalogue card and
route only by direct navigation (a bookmark, a shared link, or the E2E suite's own `page.goto`) — no
normal, discoverable path through their own sidebar, which this review found unacceptable for a report
whose whole subject is the releases Finance itself executes. Fixed: `nav-config.ts`'s "Reports" item
now requires `['reports:view', 'payroll:view']` (the same any-of shape `isNavItemVisible` already
supported for the Corrections item), and the previously-frozen `nav-config.test.ts` case was updated
to assert the corrected OR behavior rather than preserved as a stale pin — this project's own "do not
weaken existing tests" rule does not mean "never correct a test asserting behavior the approved
architecture has since superseded." A payroll:view-only Finance user now sees "Reports" in their own
sidebar, and — because every sibling catalogue card still independently gates on its own real
requirement — sees exactly the one card (Salary Release Report) they're actually authorized to open,
never any sibling report's own reports:view-only card. Proven directly: `nav-config.test.ts` gained OR
coverage for the Reports item mirroring the Corrections item's own four-case pattern, and
`26-salary-release-report.spec.ts`'s Finance permission test now asserts the sidebar link is visible
and clickable before falling back to the same direct-navigation assertions the other permission cases
in that file already use.

**Route** (`/reports/salary-release`, plus the canonical `/payroll-cycles/:cycleId/reports/salary-
release`), lazy-loaded, `RequirePermission`-gated on `[PERMISSIONS.REPORTS_VIEW,
PERMISSIONS.PAYROLL_VIEW]`. Mirrors every sibling single-Cycle report's dual flat/canonical route pair
and `useSelectedPayrollCycle` hook. No detail route. AppShell title "Salary Release Report", subtitle
"Release reconciliation for one payroll cycle — released, pending, held, and resolved entries."

**Data layer** (`hooks/use-salary-release-report.ts`) — imports the DTOs directly from
`@payroll/shared`, mirrors `use-deduction-report.ts`'s convention exactly: one list query (`enabled:
Boolean(cycleId)`, proven by a direct hook-level suite with `global.fetch` stubbed — empty Cycle issues
zero requests, a real Cycle issues exactly one, an unchanged re-render issues no duplicate), a blob-
download export function for CSV/XLSX, sorted Site ids in both the query key and URL, and a dedicated
`SalaryReleaseReportExportRowLimitExceededError` carrying the backend's structured 413 body verbatim.

**Filters** (frozen Checkpoint 1A set only): Payroll Cycle (required, single, navigation control),
Site (multi-select), Unit (disabled unless exactly one Site is selected, cleared on an incompatible
Site change, never cleared by an unrelated rerender), Row Status, Has Correction (tri-state). No
Employee search, Designation, date range, Released By, payment method, bank, or roster-status filter.
Clear Filters restores every filter to default and resets pagination, never touching the selected
Cycle.

**Release-semantics disclosure** — a concise on-screen `ReleaseSemanticsNotice` (screen-only, mirrors
`reports-payroll-summary-page.tsx`'s own `Info`-icon banner convention) states Original Released Amount
is fixed once released, and Correction Balance Payable/Recovery are later, separate settlement
amounts, never folded back in — plain wording, no `calcNet` or other implementation detail named. The
print context header carries the same disclosure as one concise extra line.

**Totals** — the backend's `SalaryReleaseReportTotals` render verbatim in two labeled groups:
**Release Amounts** (Released Amount, Pending Release Amount, Correction Balance Payable, Correction
Balance Recovery — collapse into the standard "Totals are unavailable for this result size" notice
when `totalsComputed` is `false`, never a misleading zero) and **Status** (Matching Entries, Released,
Held, Pending, No Pay Due, Recovery Due, Corrected Entries — plain DB aggregates, always exact and
always visible regardless of `totalsComputed`). No client-side summation and no `calcNet` import
anywhere in this page.

**Table** — server-paginated (`ReportPagination`), server-sorted only on the five backend-approved
fields (`employeeCode`/`employeeName`/`site`/`rowStatus`/`releasedAt`; Primary Unit, Designation,
Original Released Amount, Correction Count, and both correction balances render as plain non-
interactive headers, proven by a dedicated test). Columns, in the order the row's own release-
reconciliation narrative reads best (identity → org context → status → release specifics → correction
context): Employee Code, Employee Name, Project Site, Primary Unit (`"Name (+N more)"` for a multi-Unit
entry, never a duplicated row), Designation, Row Status, Original Released Amount, Released At,
Correction Count (badge when positive, plain `0` otherwise — the hardened sibling-report precedent),
Correction Balance Payable, Correction Balance Recovery (11 total). A non-`RELEASED` row always shows
`—` for Original Released Amount/Released At, never a fabricated figure. No Released By, Payment
Method, banking, CNIC, correction reason, or audit actor anywhere — none of these exist on the frozen
row contract in the first place. No edit action, no "View Details" action, no row click — proven both
by a direct test and by a real `GET /api/v1/reports/salary-release/:id` returning 404 in Playwright
(no such route exists). The same page-clamp safeguard every sibling report's own Checkpoint 1B work
already established is included here unchanged.

**Export UI** — Export CSV/Export Excel, mutually exclusive `activeExport` state, current Cycle/
filters/sort always applied and never just the current page, structured 413 shown via `toast.error`,
CSV/XLSX request-parity proven at the URL-construction level (identical query strings except
`format`).

**Browser Print (current-page-only scope)** — a dedicated `SalaryReleaseReportPrintOptionsDialog`/
`salary-release-report-print-fields.ts`, a fresh field vocabulary under its own versioned
`salary-release-report-print-fields:v1` `localStorage` key. Defaults to every safe card/column selected
(11 cards, 11 columns; Employee Name locked). Readability thresholds (Excellent 0–4 / Good 5–7 / Wide
8–9 / Very Wide 10+) are reused verbatim from Overtime Report's own identical 11-column ceiling. The
print context header states Cycle, Site, Unit, Row Status, Has Correction, the Original Released
Amount disclosure line, and "Current page only" — never omitting an active filter (the Deduction
Report defect this checkpoint's own authorizing instruction named explicitly as the one not to
repeat), proven directly in Playwright under `page.emulateMedia({ media: 'print' })`.

**Tests**: 4 new colocated Vitest files plus new tests in the existing catalogue test —
`salary-release-report-labels.test.ts` (6), `salary-release-report-print-fields.test.ts` (19, including
a sensitive-field sweep that deliberately does *not* forbid `correctionBalancePayable`/
`correctionBalanceRecovery` — this report's own frozen, approved fields), `use-salary-release-
report.test.ts` (21, including the direct hook-level no-request-without-Cycle suite and CSV/XLSX
request-parity), `reports-salary-release-report-page.test.tsx` (42: the OR-gate matrix — reports:view-
only/payroll:view-only/both/neither/payroll:release-only/statements:view-only — Cycle requirement,
loading/empty/error, the disclosure notice, totals including `totalsComputed: false` never rendering a
financial zero, every table column with a full sensitive-field sweep, "+N more", the correction-count
badge convention, all five statuses with no fabricated amount/timestamp on non-RELEASED rows, Original
Released Amount kept visually separate from both correction balances at once, server-only sorting,
pagination and the page-clamp safeguard, every filter including Unit compatibility clearing and an
unrelated-rerender-preserves-Unit proof, Clear Filters preserving the Cycle, export, and print context)
— 88 new tests across the four files. `reports-page.test.tsx` gained 8 new tests (Salary Release
Report card visibility across every permission combination in the OR matrix, plus the widened
page-level admit/deny boundary) without weakening any existing test. **96 new/changed frontend tests
(98 including the hostile-review sidebar fix below: `nav-config.test.ts`'s four Reports-item cases
grew to six, mirroring the Corrections item's own OR pattern). Full frontend suite: 875/875 passing.**
...
Chromium, no `page.route` interception for any RBAC or financial assertion. Covers: all five row
statuses constructed through real release/hold actions in one cycle (`RELEASED` via full worked days
and a real Unit release; `HELD` via a real hold before release; `PENDING` via a genuinely positive net
salary never released; `NO_PAY_DUE` via `eobiApplicable: false` plus zero worked days, `netSalary = 0`
exactly; `RECOVERY_DUE` via the default zero-worked-days fixture's own negative net through EOBI
alone), a dedicated multi-Unit release test (an entry spanning two real Units stays non-RELEASED until
both release, then resolves to exactly one row with one amount and one timestamp), a real post-release
Correction (submitted and approved through the real backend by a second reviewer session — self-review
is blocked — proving Original Released Amount is byte-identical before and after while Correction
Count increments), Site scoping and historical transfer (mirrors Overtime Report's own identical
pattern — `PayrollEntry.siteId` is always historical, never the employee's current Site), filters
(Unit compatibility, Row Status, Has Correction, Clear Filters preserving the Cycle), sorting/
pagination, Export (CSV/XLSX, safe headers only), Print (current-page scope, safe defaults, no
sensitive field ever offered, print body content), Responsive layout (1024px, no horizontal document
scroll), No detail action (no button/link, a direct detail-shaped request 404s), and the full OR-
permission matrix (`reports:view` alone, `payroll:view` alone via a **real seeded `FINANCE`-role**
user — never `reports:view` — both admit; neither, `payroll:release` alone, and `statements:view`
alone all deny). **15/15 passing.** Combined regression run alongside every sibling report's own
Playwright spec (`17-reports.spec.ts`, `20-project-site-payroll-report.spec.ts`,
`21-deduction-report.spec.ts`, `22-overtime-report.spec.ts`, `25-advance-recovery-report.spec.ts`) —
**63/63 passing, unweakened.**

**Request discipline observed**: zero list requests before a Cycle exists; exactly one list request
per genuine filter/sort/pagination change; zero backend requests fired merely by opening or confirming
Print; no client-side all-row fetch for totals or print; export sends exactly the current filter/sort
state with no `page`/`pageSize`.

**Known limitations, disclosed**: every field/behavior limitation already disclosed by Checkpoint 1A
(no Released By, no payment method, Original Released Amount not sortable) carries through unchanged
to the frontend, since none of it is a frontend decision to begin with. (The Reports sidebar nav item
previously stayed `reports:view`-only, leaving Finance without a discoverable navigation path — fixed
during the 2026-08-11 independent hostile review, above; no longer a limitation.)
...
## 21.10 Checkpoint 1B — Frontend, Dual-Cycle UI, Browser Print, and E2E

Frontend-only, over the frozen Checkpoint 1A backend above (`main == origin/main ==
6f8d1f7d73520182921e56b8506ca2391804f39e` verified before any change) — no backend, shared-contract,
or database change in this checkpoint. **The first Reports-module frontend needing two independent
Payroll Cycles at once**, so it deliberately does not copy any sibling single-Cycle report's own
route/hook/state pattern where that pattern assumes exactly one Cycle.

**Route and dual-Cycle URL state.** Mounted at exactly one path, `/reports/variance` (no `:cycleId`
path segment — a single path segment cannot name two independent Cycles the way every sibling
report's own `/payroll-cycles/:cycleId/reports/...` pair does), `RequirePermission`-gated on
`PERMISSIONS.REPORTS_VIEW` alone (no OR gate, matching the frozen Checkpoint 1A backend decision).
Both Cycle ids live in this route's own query string
(`?currentCycleId=...&comparisonCycleId=...`), read directly off `useSearchParams()` every render —
deliberately not mirrored into `useState` — so `currentCycleId`/`comparisonCycleId` can never
desync from each other or from the URL: a change to either always lands in the exact same commit
the other value is read in, never a window where one is stale and the other fresh. This is what
makes "never briefly display stale comparison data under the new selection" (Checkpoint 1B §4) a
structural guarantee rather than a timing accident — proven directly in
`reports-variance-report-page.test.tsx`'s own dual-cycle describe block and re-verified in the E2E
spec's arbitrary-comparison scenario. Browser Back/Forward restores a prior comparison because an
explicit user pick always pushes a new history entry (`setSearchParams` default); the one-time
smart-default assignment (below) uses `{ replace: true }` instead, so it never pollutes the Back
stack.

**Smart Comparison Cycle default, never a forced pick.** A `useEffect` fills in `currentCycleId`
(via the existing `resolveDefaultCycleId` — Draft, else newest Released, else newest Archived) and
then, only once a Current Cycle is known and no explicit `comparisonCycleId` is already in the URL,
fills in the immediately-preceding Cycle by index into the already-newest-first `cycles` list
(`resolveDefaultComparisonCycleId`, this page's own new helper). When Current Cycle is already the
single oldest Cycle in the system, this deliberately resolves to nothing — Checkpoint 1B §4's own
"where one exists" qualifier, proven by a dedicated unit test (a one-Cycle fixture) and never
silently defaulted to comparing a Cycle against itself. An already-explicit `comparisonCycleId` in
the URL (arbitrary, non-adjacent, or the same Cycle as Current) is never overridden — this is what
lets a user compare, say, the current month against the same month a year prior, not just the
immediately preceding one.

**Data layer** (`hooks/use-variance-report.ts`) — imports every DTO directly from `@payroll/shared`,
mirrors every sibling report hook's own convention. `useVarianceReportList` is `enabled:
Boolean(currentCycleId) && Boolean(comparisonCycleId)` — proven by 21 direct hook-level tests with
`global.fetch` stubbed: zero requests with either Cycle empty, exactly one request once both are
present, a second distinct request (never a silently-merged one) when Comparison Cycle changes while
Current stays fixed, and no duplicate request from an unrelated re-render. Site ids are sorted in
both the query key and the URL so an equivalent Site selection in a different pick order never
produces a second cache entry or a second network request. Export (`downloadVarianceReportExport`)
carries both Cycle refs (for the download filename only) plus every filter/sort — no `page`/
`pageSize` ever accepted, request-parity between CSV and XLSX proven directly (identical query
string except `format`). `VarianceReportExportRowLimitExceededError` carries the backend's
structured 413 body verbatim, mirroring every sibling report's identical class.

**Employee lookup** (`components/reports/variance-report-employee-lookup.tsx`) — a deliberate fork
of Employee Payroll History's own `EmployeePayrollHistoryEmployeeLookup`, over this report's own
`GET /api/v1/reports/variance/employees` endpoint (the identical `searchEmployeesByHistoricalPayroll`
contract, never a new org-wide directory). Not a shared extraction — EPH's own doc comment already
named a third historical-discovery consumer as the point to extract one; that extraction stays out
of this checkpoint's own scope, matching Salary Release Report's identical precedent of not forcing
a premature abstraction.

**Filters** (frozen Checkpoint 1A set only, plus the two Cycle selectors, which are navigation/
report-state controls, never Clear Filters targets): Site (multi-select, either-side match — a
selected Site surfaces a transfer discoverable from *either* its old or new side), Unit (disabled
unless exactly one Site is selected, cleared on an incompatible Site change), Employee, Direction,
Has Correction (tri-state). No minimum variance, roster status, release date, payment method,
correction reason, or Released By filter — confirmed absent by a dedicated unit test sweeping the
built URL for each forbidden term. **Clear Filters preserves sort** (a documented judgment call,
matching Salary Release Report's/Employee Payroll History's own established convention of leaving
sort untouched — this page never invents a narrower behavior than its own siblings already settled
on).

**Population/Direction presentation.** Two independent dimensions, rendered as two independent
badge columns — `populationStatusTone`/`directionTone`
(`components/reports/variance-report-labels.ts`): NEW is blue (info), DEPARTED is gray (neutral),
CONTINUED is green; INCREASED is green, DECREASED is red, UNCHANGED is gray. A population change is
deliberately never colored red/amber the way a real salary decrease or a hold state would — NEW/
DEPARTED are not themselves "good" or "bad." **Judgment call: Direction is rendered as its own table
column**, one field beyond Checkpoint 1B §9's own "should include" column list (which does not name
it explicitly) — given §10's own extensive treatment of Direction as an independent presentation
dimension, and the existing Direction *filter*, omitting a Direction column would leave a filtered
result with no on-screen field showing what was actually filtered on. `direction === null` (every
NEW/DEPARTED row) renders `—`, never a fabricated badge. Every missing-side financial field
(`comparisonNet`/`currentNet`/`varianceAmount`/`variancePercent`, all `string | null` off the
backend) renders through the existing shared `formatMoney` (already `—`-for-`null`) or a direct
`${value}%` guarded by an explicit `!== null` check — this page never computes a financial value
itself, only formats what the backend already computed.

**Transfer/Unit/Correction context** — `siteTransfer`/`unitChange`/`hasComparisonCorrection`/
`hasCurrentCorrection` render as independent Yes/No (amber-badged when Yes) columns, never folded
into or replayed against the compared Net figures — proven directly (a correction flag toggled via
the Has Correction filter never changes the same row's own Previous/Current Net cell text) in both
the unit suite and the E2E spec's own Scenario J.

**Totals** — the backend's `VarianceReportTotals` render verbatim in two labeled groups: **Financial
Totals** (Comparison Total Net, Current Total Net, Aggregate Variance, Aggregate Variance % —
collapse into the standard "Totals are unavailable for this result size" notice when
`totalsComputed` is `false`, never a misleading zero) and **Population & Direction** (Matching
Employees, New, Departed, Continued, Increased, Decreased, Unchanged, Site Transfers, Corrected
Entries — always exact, regardless of `totalsComputed`, matching the backend's own frozen
Checkpoint 1A totals contract). Aggregate Variance % is rendered as the single backend-provided
figure, never averaged/summed from individual rows' own `variancePercent` — this page has no code
path that could do that summation even by accident, since it never touches individual rows when
building the totals cards.

**Sorting** — exactly the backend-approved `VARIANCE_REPORT_SORT_FIELDS` (`employeeCode`,
`employeeName`, `populationStatus`, `comparisonSite`, `currentSite`, `comparisonNet`, `currentNet`,
`varianceAmount`) are clickable `SortableHead` buttons with `aria-sort`; `variancePercent`,
`direction`, `siteTransfer`, `unitChange`, and both correction columns render as plain,
non-interactive headers — proven directly (no `<button>` inside those header cells). No client-side
re-sort after the response arrives.

**Pagination/page clamp** — the shared `ReportPagination` component, backend-provided `page`/
`pageSize`/`total` verbatim, plus the same page-clamp safeguard every sibling report's own page
already established (`page > lastValidPage` → `setPage(lastValidPage)`, keyed off `report.data`
only, never `isLoading`/`isFetching`, so it can never clamp against a stale response).

**Print** (`variance-report-print-fields.ts`/`variance-report-print-options-dialog.tsx`,
`variance-report-print-fields:v1` localStorage key) — a fresh field vocabulary (13 summary-card
fields, 14 table-column fields including `direction`), current-page-only, browser print only, no
backend PDF, no additional data request (proven directly in both the unit suite and the E2E spec —
zero `/api/v1/reports/variance` requests between opening Print Options and the browser print call).
Print context explicitly includes both Cycles (`Current: ... `/`Compared with: ...`), Site, Unit,
Employee, Direction, Has Correction, and the concise formula line `"Variance = Current Net −
Comparison Net"` — never omitting an active filter from the printed header (the Deduction Report
defect this project's own standing checkpoint instructions explicitly name). Readability thresholds
reuse Employee Payroll History's own 13-column-ceiling values verbatim (Excellent 0–4 / Good 5–7 /
Wide 8–10 / Very Wide 11+) since both this report's 14-column maximum and EPH's own 13-column
maximum land in the same "Very Wide" tier.

**Employee Payroll History drill-down** — "View Full History" per row navigates to
`/reports/employee-payroll-history` via `location.state.listState` (the exact mechanism EPH's own
list page already reads on mount), carrying `employeeId` plus a synthesized
`EmployeePayrollHistoryEmployeeOption`-shaped candidate (`cnic: null`, since this row never carries
one; site fields fall back across sides for a `DEPARTED` row with no current side) so EPH's own
lookup chip displays immediately, with no second lookup round trip. No new Variance-specific detail
query, no duplicate correction/history page — proven directly (the mocked list hook is never called
again after the navigation) and in the E2E spec (real navigation lands on the real EPH page,
pre-filtered to the correct employee).
...
new frontend tests, all passing** (corrected during independent hostile review — the original count
of 104/"9 labels" mismatched the actual 5 tests in `variance-report-labels.test.ts`). Full frontend
suite: 1002/1002 passing on three consecutive full-suite runs during review (the previously-reported
single `payroll-entry-grid.test.tsx` failure did not reproduce; no code linkage exists between the
Variance diff and that file). `typecheck`
(all workspaces), `lint` (0 errors, only pre-existing unrelated warnings), `build`, `typecheck:e2e`,
and `git diff --check` all clean.

**E2E** (`tests/e2e/specs/29-variance-report.spec.ts`, 10 tests, all passing against the real
backend) — a real month-end rollover (Finalize + Archive-and-create-next, the same real construction
`19-employee-payroll-history-frontend.spec.ts` already established) produces two genuine Payroll
Cycles, then one comprehensive scenario proves New/Departed/Continued
(Increased/Decreased/Unchanged), a zero-Comparison-Net row (no Infinity/NaN), a real Site Transfer
(constructed by patching the employee's Site *before* rollover, so the Comparison-side entry stays
pinned to the old Site while the Current-side entry — freshly materialized by rollover — reflects
the new one), a real Unit Change, and real correction context (approved via a second reviewer
session, never the requester). Separate tests cover two-sided Site authorization (a Site-A-only
user sees a same-Site row in full but never the Transfer row, whose Current side sits at Site B),
export (CSV/XLSX, safe headers only), print (current-page scope, both Cycles in the printed context,
zero report requests), the Employee Payroll History drill-down, the Reports catalogue card plus the
no-detail-endpoint confirmation, and the permission gate (`reports:view` alone admits; `payroll:
view` alone — unlike Salary Release Report — is denied, proving Variance really has no OR gate).
Combined regression run with `17-reports.spec.ts`/`19-employee-payroll-history-frontend.spec.ts`/
`26-salary-release-report.spec.ts`: **39/39 passing**, including this spec running immediately after
19's own rollover of the shared Draft cycle — proving Variance's own real second rollover composes
correctly with an already-cycle-advanced shared suite database.

**Files changed**: `frontend/src/hooks/use-variance-report.ts` (new),
`frontend/src/hooks/use-variance-report.test.ts` (new),
`frontend/src/components/reports/variance-report-labels.ts` (new),
`frontend/src/components/reports/variance-report-labels.test.ts` (new),
`frontend/src/components/reports/variance-report-print-fields.ts` (new),
`frontend/src/components/reports/variance-report-print-fields.test.ts` (new),
`frontend/src/components/reports/variance-report-print-options-dialog.tsx` (new),
`frontend/src/components/reports/variance-report-employee-lookup.tsx` (new),
`frontend/src/routes/reports-variance-report-page.tsx` (new),
`frontend/src/routes/reports-variance-report-page.test.tsx` (new),
`frontend/src/App.tsx` (additive: one lazy import, one route object),
`frontend/src/routes/reports-page.tsx` (additive: one catalogue entry),
`tests/e2e/specs/29-variance-report.spec.ts` (new). No existing report's own file was modified
beyond the two purely-additive edits above. No backend, shared-contract, or database file touched.

**No Dashboard or unrelated report work in this checkpoint.** **Not committed, not pushed, not
deployed** — stopped deliberately for independent hostile review, per explicit instruction.
