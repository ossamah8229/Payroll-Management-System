...
| **Statements** | (read/aggregation only) | Per-employee ledger: earnings, deductions, corrections, balance adjustments, and advances across cycles, with three independent running balances (Payable to Employee / Recoverable from Employee / Advance Outstanding — never combined into one figure). Reads Payroll Entry, Corrections, Balance Adjustments, and Advances; owns no primary data of its own. **Phase 7A, Checkpoint 1 — COMPLETE, 2026-07-27 (backend ledger only, `backend/src/modules/statements/`, no frontend yet): full design and event-kind/sign-convention record in `docs/architecture/workflows/statements-ledger.md`.** |
| **Reports** | (read-only queries) | Fines & EOBI Report and future reports. Each report is an isolated, side-effect-free query module — this is also the extensibility seam for future reports (see below). **New note, 2026-07-11**: once both modules exist (Phase 7), Reports should reuse Statements' own ledger-computation/aggregation code rather than independently re-deriving the same per-employee/per-cycle figures a second time. |
| **Dashboard** | (read-only, derived) | **Checkpoint 1A (backend), 2026-08-13:** a single aggregation endpoint (`GET /api/v1/dashboard`, `docs/architecture/workflows/dashboard.md`) answering "what does the active/current payroll cycle look like right now, and is there anything I need to pay attention to?" — Total Employees, Net Payroll, Pending Release, Release Progress, Deduction Breakdown, a compact Top-5 Site Payroll Summary, and Attention Required (Held Entries, Pending Corrections, Recovery Due). Route gate is the established any-of mechanism (`reports:view OR payroll:view`, no new permission), but every widget independently re-checks its own permission — a caller passing the outer gate does not automatically see every widget. Reuses Payroll Summary's own aggregation (`reports.service.ts`'s `buildPayrollSummaryData`) for every cycle-scoped financial figure rather than a second calculation; no new financial formula anywhere in this module. No caching layer yet — the previous "(read-only, cached)... candidate for short-TTL caching" description here was aspirational only; this checkpoint deliberately does not build it (frontend React Query caching is Checkpoint 1B's own concern). Frontend (Checkpoint 1B) is Not Started. |
