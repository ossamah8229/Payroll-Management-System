...
payroll data remains server-side permission- and site-filtered exactly as before; historical
export/download filenames include the payroll period.

**Backend** (small — the backend already had no "current cycle" concept; every route already took an
explicit `cycleId`): `assertEntryEditable` (`payroll-entry.service.ts`) extended to also reject once
`cycle.status === 'ARCHIVED'`, alongside its existing `released` check — see
`docs/architecture/database/payroll-entry.md`'s Immutability note for the full per-surface breakdown
(single-entity update/delete, work-line add/update/delete, `bulkUpdatePayrollEntries`,
`importPayrollEntries`, and `deferAdvanceSchedule`, which inherited the extension with zero code
change since it already includes the source entry's `cycle` relation). `listPayrollCycles` gained a
derived `isCurrentDraft` boolean (`cycle.status === 'DRAFT'`, no schema change, no extra query). Export
filenames on Bank Sheet, Cash Receiving (already period-aware, no change needed), single Payslip PDF,
and Payslip batch ZIP all now include the cycle's `{year}-{month, zero-padded}` period slug (e.g.
`bank-sheet-hbl-2026-07.csv`, `payslip-emp001-2026-07.pdf`, `payslips-2026-07.zip`) — file *contents*
unchanged.

**Frontend:** flat routes (`/payroll-entry`, `/release-salary`, `/bank-sheet`, `/cash-receiving`,
`/payslips`) kept as-is and joined by five nested siblings at `/payroll-cycles/:cycleId/...` rendering
the identical page component; a new shared `useSelectedPayrollCycle(routeSuffix)` hook (
`frontend/src/hooks/use-selected-payroll-cycle.ts`) resolves the route's `cycleId`, and — only on the
flat route, only once cycles have loaded — replaces the URL with the resolved default
(`resolveDefaultCycleId`: newest Draft → newest Released → newest Archived → no redirect if the list
is empty); an explicit but nonexistent/malformed `cycleId` in the URL is deliberately **not**
silently redirected, so it falls through to the page's own not-found/empty handling. A shared
`<PayrollCycleSelectField>` + `<PayrollCycleStatusBadge>` pair (`components/payroll-cycle/
payroll-cycle-selector.tsx`, `docs/design-system.md §2.6`) replaces what had been three independent,
duplicated ad hoc local selectors on Bank Sheet, Cash Receiving, and Payslips. Payroll Entry gained an
`ArchivedReadOnlyBanner` (`docs/design-system.md §2.5`) and now disables/hides edit, delete, bulk
update, import, hold-toggle, and work-line editing whenever the selected cycle is `Archived`, while
keeping filtering/export/navigation live; a genuinely dormant bug in its own `isEntryEditable` helper
(stricter than the backend, requiring `cycleStatus === 'DRAFT'` even though a held/unreleased entry in
a `Released` cycle was always meant to stay editable) was found and fixed as part of making non-Draft
cycles reachable for the first time. Salary Release now supports Draft (Finalize + rollover,
unchanged), Released (rollover only, gated to the current outgoing cycle), and Archived (read-only
summary, no Finalize/Rollover control ever rendered for a historical selection); all open confirmation
modals close automatically on cycle change; a successful rollover navigates directly to
`/payroll-cycles/{newDraftId}/release` rather than relying on the default-selection redirect. Bank
Sheet, Cash Receiving, and Payslips all now use the shared selector in place of their own; Payslips
additionally resets its batch selection on cycle change. React Query cache keys were **not**
redesigned — the existing `cycleId`-aware keys already isolated per-cycle data correctly.

**Tests:** 8 new backend tests (`backend/tests/payroll-cycle-archived-lock.test.ts`) covering Archived
rejection across every mutation surface, an unchanged Released regression (held entry still editable),
and `isCurrentDraft` correctness across a mixed Draft/Released/Archived list; filename assertions added
to `bank-sheets.test.ts`, `cash-receiving.test.ts`, and `payslips.test.ts` (single PDF + batch ZIP).
7 new frontend unit tests (`use-payroll-cycles.test.ts`) covering `resolveDefaultCycleId`,
`formatCyclePeriodSlug`, and `formatCycleOptionLabel` — full frontend suite **21/21** (14 prior + 7
new). Frontend component rendering tests remain out of scope (no existing convention for them in this
codebase — pure-function tests only); real-stack/browser verification performed via the compiled
server + `curl` in the absence of a browser-automation tool, documented explicitly as a coverage gap
rather than silently skipped.

**Security correction (2026-07-16, before commit) — `passwordHash` response leak, found during final
review, not a Checkpoint 4 regression.** Requested as a "Payroll Cycle response serialization" fix;
investigation instead traced the confirmed defect to `backend/src/modules/users/users.service.ts` —
`listUsers`/`getUser`/`createUser`/`updateUser` returned the raw Prisma `User` row (including
`passwordHash`) straight into `GET/POST/PATCH /api/v1/users` and `GET /api/v1/users/:id`. Fixed with
an explicit `select` (`USER_SUMMARY_SELECT`, never fetches `passwordHash` for these read paths at
all) and DTO assembly (`toUserSummary`) matching the frontend's own already-narrow `ManagedUser`
shape. The requested narrow review of every directly-related Payroll Cycle/Backup Package/Salary
Release response (`GET/POST /payroll-cycles`, finalize, rollover, Backup Package list/detail, the
Salary Release unit-status payload) confirmed those were **already clean** — `PayrollCycle`'s
`createdBy`/`releasedBy`/`archivedBy` are plain scalar FK strings, never an included `User` relation,
and the one place a `User` relation is queried (`payroll-release.service.ts`'s
`getUnitReleaseStatus`) already narrowed it to `{ id, name }` before returning it. New permanent
convention recorded: `docs/architecture/system-conventions.md §4`, "No HTTP route may return a raw
Prisma model or relation object." **10 new regression tests**: 4 in `users.test.ts` (pinning the
direct fix — create/list/detail/update responses, plus a database-level confirmation the stored hash
is a real argon2 hash never echoed back) and 6 in the new
`backend/tests/payroll-lifecycle-response-security.test.ts` (the negative-finding confirmation, using
a new shared `assertNoSensitiveKeys()` recursive-key helper in `tests/helpers.ts`, across cycle
list/detail, Finalize, rollover, Backup Package list/detail, and the Salary Release unit-status
payload, spanning Draft/Released/Archived cycles). Full backend suite: **487/487** (477 prior + 10
new). Live-verified a second time against a freshly compiled server: `passwordHash` confirmed absent
from every Users route response; the full Checkpoint 4 Archived-lock matrix (single-entry, bulk,
work-line, and import mutation surfaces, across a held-then-archived entry and a
released-then-archived entry, distinguishing the two different rejection messages by precedence) was
independently reconfirmed end-to-end across a 4-cycle Draft→Released→Archived chain. One further,
non-blocking observation surfaced during this pass (not fixed, out of the requested narrow scope): a
malformed (non-UUID) `cycleId` in the URL produces a raw `PrismaClientKnownRequestError` message
(including an absolute filesystem path) in the 500 response body — but only when `NODE_ENV !==
'production'` (`backend/src/common/middleware/error-handler.ts`'s existing, deliberate
`isProduction` gate already masks it to a generic "Something went wrong" in real production) — a
...
`docs/PROJECT_PROGRESS.md` §1's own Checkpoint 6 entry for the full record.

**Checkpoint 6A (Corrections Navigation Permission Verification & Focused Fix) is also complete** —
review-only, found and fixed one real gap: the Corrections sidebar item was gated on `payroll:entry`
alone, invisible to a `corrections:approve`-only reviewer even though the Review Queue behind it is
authorized for exactly that permission. Frontend-only fix (`NavItem.requiredPermission` broadened to
accept an OR-array); no backend change, no new permission key, no schema change.

**Checkpoint 7 (End-to-End Financial Lifecycle Validation, Audit Hardening & Phase 6 Close-Out) is
also complete, and Phase 6 is now CLOSED.** A full validation pass over every Checkpoint 1–6A
lifecycle found the audit trail, API/error consistency, permissions, reporting, and exports already
correct — but found one genuine, load-bearing correctness gap: the `ACTIVE -> CONSUMED`
materialization transition every prior checkpoint above deliberately deferred ("a later checkpoint's
own event") had never been built, which meant a materialized `DEFERRED PAYABLE`/`RECOVERY` obligation
could never actually reach `SETTLED` through any supported workflow — Checkpoint 5A's own
`RESERVED_AMOUNT_UNAVAILABLE` ceiling correctly blocked double-processing an active reservation, but
nothing ever resolved that reservation either. Fixed using the `settlementId`/`consumedAt` columns
`BalanceAdjustmentMaterialization` had carried unused since Checkpoint 5's own schema (no migration):
`payroll-release.service.ts`'s `releaseProjectUnit` — the exact moment a `PayrollEntry` transitions
`released: false -> true` — now also acquires `lockPayrollCycleForUpdate` (a third participant in
Checkpoint 5's documented "cycle, then adjustment" lock order) and consumes every `ACTIVE`
materialization reserved against the entries it just released: one `BalanceAdjustmentSettlement` per
materialization, `remainingAmount` decremented, `.status -> SETTLED` once it reaches zero, the
materialization itself flipped `ACTIVE -> CONSUMED`, all inside that same release transaction. A
companion eligibility fix (`TARGET_ENTRY_ALREADY_RELEASED`) closes the race this makes load-bearing:
a manual materialization can no longer target an already-released `PayrollEntry`. See
`docs/architecture/workflows/corrections-and-balance-adjustments.md`'s own Checkpoint 7 scope note for
the full analysis, and `docs/PROJECT_PROGRESS.md` §1's own Checkpoint 7 entry for verification
results. No schema migration, no new permission key, no new financial lifecycle — this closes the
existing, already-designed-for lifecycle's own final link. Phase 7 has not been started.
...
**Checkpoint 2 — Statements frontend page — IMPLEMENTED, 2026-07-28, awaiting review, NOT
COMMITTED.** `frontend/src/routes/statements-page.tsx`, `frontend/src/hooks/use-employee-statement.ts`,
`frontend/src/components/statements/statement-labels.ts` — a purely presentational consumer of
Checkpoint 1's ledger endpoint, no backend/shared change, no migration. Site → Unit → Employee
selection reuses the existing `EmployeeLookup`/`useAccessibleProjectSites`/`useProjectUnits` building
blocks rather than a second filtering system; Statement Period is a `PayrollCycle` range only
("Latest 12 Cycles" default or an explicit From/To pair), never an arbitrary date range. Opening/
Closing Balances and every ledger row's running balances are rendered exactly as the backend
returns them — zero frontend financial recalculation. Informational rows render "Informational,"
never an invented monetary figure; the Advance-history restriction notice renders on
`scope.advanceHistoryIncluded === false`, worded to communicate restricted visibility, never "no
advances." No Print/Excel export, per this checkpoint's own explicit scope boundary. Frontend suite
**172/172** (50 new/extended: `statement-labels.test.ts`, `use-employee-statement.test.ts`,
`statements-page.test.tsx`, four new `nav-config.test.ts` cases); typecheck/lint/build clean; a new
permanent E2E spec (`tests/e2e/specs/15-statements.spec.ts`, 3/3) verified navigation, the real
selection workflow, a real financial-movement row and a real informational row rendering correctly
with correct running balances, and that viewing a Statement issues only `GET` requests, against a
freshly provisioned real backend/database/production frontend build. One discrepancy discovered and
disclosed (not silently patched): the Advance-restriction-notice's own real trigger scenario is
unreachable through the Employee Lookup for any non-Master-Admin session, since employee search
matches only an employee's *current* site — see `docs/PROJECT_PROGRESS.md`'s "Phase 7A, Checkpoint 2"
entry for the full explanation and recommended follow-up. **Print/Excel export, Reports, and
Dashboard remain separate, later checkpoints requiring their own explicit authorization.**

**Checkpoint 2 correction — historical employee discovery — IMPLEMENTED, 2026-07-28 (same day),
awaiting review, NOT COMMITTED.** A dedicated architectural investigation (approved before any code
changed) confirmed the Advance-restriction reachability gap above was a real, fixable gap, not dead
functionality — recommended Option A: redesign Statements' own employee selection, leave the
general Employee Lookup untouched. New `GET /api/v1/statements/employees`
(`statements.service.ts`'s `searchStatementEmployees`) discovers employees via historical
`PayrollEntry.siteId`/`PayrollEntryWorkLine.unitId` (the same pattern `payslips.service.ts`/
`payroll-entry.service.ts` already use), never `Employee.siteId`; `employees.service.ts`/
`frontend/src/components/ui/employee-lookup.tsx` are completely untouched (confirmed by
`employees.test.ts`/`advances.test.ts`/`corrections-settlement.test.ts`, 114/114). A new dedicated
`StatementEmployeeLookup` component (not a flag on `EmployeeLookup`); the Statements page redesigned
Employee-first, Site/Unit now optional narrowing filters. No schema/migration change, no new index.
The Advance-history restriction deliberately still follows current site (`Advance` has no
historical site attribution). The mocked E2E restriction test is replaced with a genuine Site A →
Site B transfer scenario — no interception. `statements.test.ts` **37/37** (9 new); frontend suite
**180/180**; E2E **3/3**; typecheck/lint/build clean across shared/backend/frontend. Full record:
`docs/PROJECT_PROGRESS.md`'s "Phase 7A Checkpoint 2 — architectural investigation and correction"
entry; full design record: `docs/architecture/workflows/statements-ledger.md §16`.

**Phase 7A — COMPLETE, reviewed and approved, 2026-07-28.** Both checkpoints above, plus the
same-day correction, are reviewed, approved, committed, pushed, and deployed. Phase 7A now
includes: the canonical Statement backend ledger (Checkpoint 1); the Statements frontend page
(Checkpoint 2); historical employee discovery via `PayrollEntry.siteId` (Checkpoint 2 correction);
Employee-first Statement selection; a naturally-reachable Advance-history restriction workflow
(no mocking); and full test coverage across backend, frontend, and E2E. Full landing record:
`docs/PROJECT_PROGRESS.md`'s "Phase 7A — closure and landing" entry. **Phase 7 as a whole is NOT
complete — Phase 7B (Statement print/export), 7C (Reports), and 7D (Dashboard) are all Not Started,
each requiring its own separate, explicit authorization before work begins.**
