...
| **Row action menu (`⋯`)** | n/a (Employee Registry's own pre-existing pattern, formalized as a reusable convention — Employee Row Actions, UAT 2026-08-11) | A compact `MoreHorizontal` icon-button trigger opening a `<DropdownMenu>` (`components/ui/dropdown-menu.tsx`, Radix), always the row's **last/trailing control** — never before an identity column, never a floating/hovering control, never permanently expanded. Accessible name `"<Action noun> for <row's own identifying name>"` (e.g. `"Employee actions for Jane Doe"`); no menu item communicated by icon/color alone. When the row's own action requires a permission the current user lacks, the trigger renders nothing at all (never a visible-but-dead/403-on-click action) — the reserved trailing slot still renders, empty, matching this same convention in every table that uses it. For an ordinary `<Table>` (Employee Registry's own usage) this is simply the row's last `<TableCell>`, in normal document flow. **Historical note, corrected 2026-08-12**: Payroll Entry's own first implementation (UAT 2026-08-11) made this a dedicated `actions` entry in `PAYROLL_COLUMNS`, promoted to `position: sticky; right: 0` because that grid's ~26 columns already sum past 2,300px. Further UAT feedback was that the `⋯` menu is a *characteristic of the row*, not a payroll data column, and that a second sticky-right pane wasn't the desired UX — see the **Trailing row action** entry below for Payroll Entry's corrected, current implementation. The git history around that 2026-08-11 commit still reflects the original sticky-right version; treat this table row as the current, superseding guidance. |
| **Trailing row action (in-flow, non-sticky)** | n/a — Payroll Entry Employee Row Actions, row-level-control correction, UAT 2026-08-12 | Payroll Entry's own corrected `⋯` implementation: rendered by `payroll-entry-row.tsx` as a trailing flex sibling *after* the row's own data-cell grid (`<div data-row-action>…</div>`), never as a `PAYROLL_COLUMNS`/`gridTemplateColumns` entry — it does not participate in column count, sorting, visibility, or totals-calculation logic, and is **not** `position: sticky`. It scrolls in normal horizontal flow with the rest of the row, exactly like `netSalary` immediately to its left — off-screen at scroll position 0, reachable only by scrolling right to the grid's own end. `PayrollEntryGrid` reserves the same fixed trailing width (`columns.ts`'s `ROW_ACTION_WIDTH`) across the header/group-header/totals rows too, purely as blank margin, so their own right edge lines up with where a body row's own trailing control sits — that reserved space is deliberately not a column either. Use this pattern (not the sticky-right pattern above) whenever a row-level action must stay visually attached to its row rather than pinned to the viewport. |
| **Frozen identity pane (sticky-left)** | n/a — Payroll Entry Frozen Employee Identity Pane UAT correction, 2026-08-12 | **Data-entry safety decision**: employee identity remains frozen while horizontally scrolling Payroll Entry so payroll values cannot be visually detached from the employee they belong to. Payroll Entry's ~26-column, 2,300px+ grid used to let the employee's name/code scroll out of view along with every other column — a material risk of entering a payroll value against the wrong employee, since the values directly affect salary calculations. Fixed by making `employeeCode`/`employeeName` — the minimum column pair that unambiguously identifies an employee (name alone is insufficient; two employees can share a name) — `position: sticky; left: <offset>px`, pinned to the LEFT of the grid's single scroll container while every other payroll column scrolls underneath (`columns.ts`'s `FROZEN_LEFT_COLUMN_IDS`/`stickyIdentityCellClassName`/`stickyLeftOffsets`). Deliberately excludes `serial`/`status` (before it) and `designation`/`site`/... (after it) — freezing more than the minimum needed to identify the employee was out of scope. This is the grid's **only** frozen/sticky-positioned region — the row's own trailing `⋯` action (see the row above) is deliberately not a second frozen pane, so there is only ever one thing to reason about pinned to the viewport at a time. **Layering correction, UAT 2026-08-12b**: `stickyIdentityCellClassName` gained a mandatory `z-10` after a production report of scrolling content (a sort chevron) painting over the pane — every layer that renders `employeeCode`/`employeeName` shares it, so a scrolling sibling's document-order tie-break can never win. **Focus-ring containment correction, UAT 2026-08-14**: the `z-10` fix protects a scrolling cell's own background/border/text, but not a *focus ring* — `box-shadow`/`outline` are pure paint, invisible to both hit-testing and normal stacking-order comparisons, and can render a few px beyond their own cell's box; since a virtualized row is `position: absolute` with no z-index of its own, a ring that escapes its cell is unclipped paint that can land on a neighboring row's identity pane regardless of z-index. **The general invariant, stated once**: all horizontally scrolling Payroll Entry content must paint beneath the frozen Employee Code/Employee Name pane, including badges, borders, focus states, sort controls, and other overflow visuals. Enforced per-cell (never on the row itself — see below) via `overflow-hidden` on every scrolling cell that holds a focusable control (`payroll-entry-row.tsx`), `self-stretch` so that cell's own box is the full row height (not shrunk to its content, which would leave no headroom to clip into), and `focus:ring-inset` for the dense `w-full` text/number inputs specifically (`inline-cells.tsx`'s `baseInputClassName`) — those have no spare margin at all for an outward ring, so the ring is drawn inside the border box instead. **A dead end worth recording**: `overflow-hidden` on the row's own outer container (rather than per-cell) looked like the simpler, more general fix, but breaks `position: sticky` outright — `overflow: hidden` establishes a *scroll container* per the CSS Overflow spec, and since this row never itself scrolls (only the outer `role="table"` container does), it silently becomes the sticky cells' nearest such ancestor instead of the real one, turning them into plain statically-positioned cells that scroll away with everything else. Verified by real pixel screenshot comparison and a geometric ring-containment proof (`tests/e2e/specs/28-payroll-entry-frozen-identity.spec.ts`'s Scenario K) — `elementFromPoint` hit-testing (Scenario J's own method) structurally cannot see a ring/outline bleed at all, since it queries layout-box topology, never painted pixels. **Vertical-edge-coverage correction, UAT 2026-08-15**: a third, genuinely different bleed survived both prior fixes — a *static*, unfocused ~1px sliver of a scrolling cell's own border (the Units pill, any bordered input) visible above and below the frozen employee name, reproducible with no hover/focus at all. Root cause: `employeeCode`/`employeeName` (`ReadOnlyCell`, `inline-cells.tsx`) sized to their own content height (~24px) and were vertically centered *within* the row by the row's own `items-center` — the same as every ordinary (non-identity) `ReadOnlyCell` — while a bordered scrolling control in the same row is taller (`self-stretch`, the 2026-08-14 fix, ~26px including its own 1px border), centered on the same row midpoint. Both centered on the same point but one shorter than the other leaves the frozen cell with *no painted box at all* at the row's own top/bottom edges — not a stacking-order tie `z-10` can win (the boxes don't overlap there to begin with) and not a focus-ring escape `overflow-hidden`/`ring-inset` can contain (the border is the control's own static, always-rendered box, not paint escaping it). Invisible to both prior fixes' own regression coverage: Scenario J's hit-test sweep samples only each row's vertical *center*; Scenario K's ring-containment proof only exercises a *focused* control. Fixed via `ReadOnlyCell`'s new `fullHeight` prop (`inline-cells.tsx`), applied only to `employeeCode`/`employeeName` in the body row and to the totals row's own identity cells (`payroll-entry-row.tsx`, `payroll-entry-totals-row.tsx`) — `self-stretch` makes the cell's own box the row's full height, `flex items-center` re-centers its content inside that taller box, and (when `truncate` is also set) an inner `min-w-0` wrapper span keeps ellipsis truncation working despite the outer flex container (verified live: a flex item's default `min-width: auto` silently defeats `text-overflow: ellipsis` without it). Deliberately **not** folded into `stickyIdentityCellClassName` itself, unlike the z-10/border-r treatment above — that helper is also shared by the group-header and column-header rows, which already get full-height coverage for free from their own containers' default `align-items: stretch` (no `items-center` override there, verified live), and forcing `flex`/`self-stretch` onto them too would risk the same flex-vs-`truncate` pitfall for no benefit. **The invariant, extended**: the frozen pane's own painted box must cover the *entire* row-height band a scrolling neighbor can occupy, top edge and bottom edge, not only wherever it happens to tie in stacking order or whatever a focus ring might escape into. Verified by real, measured `getBoundingClientRect()` geometry (`frozenTop <= controlTop && frozenBottom >= controlBottom`, checked at every row, not just the center) across 16 real rows × 7 control types in real Chrome, and by new Scenario L in `28-payroll-entry-frozen-identity.spec.ts` (40-employee virtualized dataset, top/bottom/recycled row sets, no focus/hover involved) — run 3× clean. |
...
| **"Name (CODE)" select option labels** | n/a — established by the Bank select (`employee-form-modal.tsx`'s `{bank.name} ({bank.code})`); formalized as a shared convention, Unit selector UAT correction, 2026-08-14 | A native `<select>`'s own business-identifying record (Bank, Project Unit/branch) shows as `Name (CODE)` in both its options and its own closed/selected value — the code always read from that record's own authoritative field (`Bank.code`/`ProjectUnit.code`), never typed by the user, guessed from the name, or duplicated into a frontend lookup table. Falls back to the plain name alone when the record's own code is null (`ProjectUnit.code` is a genuinely nullable field) — never a bare `"(null)"`/`"(undefined)"`. A native `<option>`'s text content can't carry its own nested markup, so there is no muted/secondary color for the code specifically here (unlike, say, a badge) — this is the deliberate, accepted tradeoff for using the platform's own `<select>` rather than a custom combobox. `SiteUnitSelect` (`frontend/src/components/ui/site-unit-select.tsx`) is this convention's second, and currently only other, application — the one shared Site→Unit cascading selector both Employee Registry's own create/edit form and Payroll Entry's "New Employee" quick action render through (`EmployeeFormModal`), so the two can never independently diverge on this. |
