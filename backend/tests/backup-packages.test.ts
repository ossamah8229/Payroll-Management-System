...
/** Redacts the live "Generated On: <date>, <time>" field so two exports produced at different
 * instants can still be compared byte-for-byte on everything else. `generatedAt.toLocaleString
 * ('en-US')` (cash-receiving.service.ts / statements.service.ts) always renders as
 * "M/D/YYYY, H:MM:SS AM/PM" — the regex matches that whole shape (not just up to the first comma)
 * so the date, the comma, and the time all get redacted together. */
function stripGeneratedOn(text: string): string {
  return text.replace(/Generated On: \d{1,2}\/\d{1,2}\/\d{4},\s\d{1,2}:\d{2}:\d{2}\s?[AP]M/g, 'Generated On: <redacted>');
}
...
describe('Phase 5 Checkpoint 2 — Backup Packages', () => {
  ...
  it('the backup Cash Receiving CSV rows are byte-identical to the live Cash Receiving export (generated-at excluded)', async () => {
    ...
    // The redaction itself must actually have fired, and must have consumed the whole live
    // timestamp — not just the date half up to the first comma — otherwise the equality above
    // would pass or fail by luck depending on whether both calls landed in the same second.
    expect(backupContent).toMatch(/Generated On: \d{1,2}\/\d{1,2}\/\d{4}, \d{1,2}:\d{2}:\d{2}\s?[AP]M/);
    expect(stripGeneratedOn(backupContent)).not.toMatch(/\d{1,2}:\d{2}:\d{2}\s?[AP]M/);
  });

  it('stripGeneratedOn redacts the full "M/D/YYYY, H:MM:SS AM/PM" field, not just the date up to the first comma', () => {
    // Single- and double-digit month/day/hour, plus the AM/midnight and PM/noon boundaries —
    // every shape `Date#toLocaleString('en-US')` actually produces for this field in production.
    const cases = [
      'Generated By: A. Admin,Generated On: 8/15/2026, 9:41:23 AM',
      'Generated By: A. Admin,Generated On: 12/1/2026, 11:05:00 PM',
      'Generated By: A. Admin,Generated On: 1/1/2027, 12:00:00 AM',
      'Generated By: A. Admin,Generated On: 1/1/2027, 12:00:00 PM',
    ];
    for (const line of cases) {
      const redacted = stripGeneratedOn(line);
      expect(redacted).toBe('Generated By: A. Admin,Generated On: <redacted>');
      // Old (buggy) behavior stopped at the first comma and left the time fragment behind.
      expect(redacted).not.toMatch(/\d{1,2}:\d{2}:\d{2}/);
    }
  });
