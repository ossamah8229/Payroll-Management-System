...
import type { AdvanceRecoveryReportEmployeeCandidate } from '@payroll/shared';
import { cn } from '@/lib/cn';
import { useAdvanceRecoveryReportEmployeeSearch } from '@/hooks/use-advance-recovery-report';

const SEARCH_DEBOUNCE_MS = 300;
const MIN_SEARCH_LENGTH = 1;

function candidateDisplayLabel(candidate: Pick<AdvanceRecoveryReportEmployeeCandidate, 'name' | 'employeeCode'>): string {
  return candidate.employeeCode ? `${candidate.name} (${candidate.employeeCode})` : candidate.name;
}

/**
 * Advance Recovery Report Checkpoint 1B — this report's own dedicated employee picker, over
 * `GET /api/v1/reports/advance-recovery/employees`. Authorized/scoped by CURRENT `Employee.siteId`
 * (the backend's own doc comment) — **deliberately not** `EmployeePayrollHistoryEmployeeLookup`
 * (historical `PayrollEntry.siteId` discovery) and not the plain current-site `EmployeeLookup`
 * (Advances/Corrections' component, which hits the unrelated org-wide `/employees` search rather than
 * this report's own purpose-built, differently-shaped candidate endpoint). A third independent fork
 * of the same debounced-listbox interaction shape, matching this module's own established precedent
 * of each report owning its own lookup component rather than sharing one across disclosure classes.
 */
export function AdvanceRecoveryReportEmployeeLookup({
  id,
  value,
  selectedCandidate,
  onChange,
  ...
  onChange: (employeeId: string, candidate: AdvanceRecoveryReportEmployeeCandidate | undefined) => void;
  /** Optional narrowing filter — current `Employee.siteId`. */
  siteId?: string;
  placeholder?: string;
  disabled?: boolean;
}) {
  ...
  const results = useAdvanceRecoveryReportEmployeeSearch(
    { search: debouncedQuery.trim(), siteId, pageSize: 25 },
  ...
  function selectCandidate(candidate: AdvanceRecoveryReportEmployeeCandidate) {
        ...
        >
          ...
          {matches.map((candidate, index) => (
            ...
            >
              ...
              <span className="text-[10.5px] text-text-muted">{candidate.siteName}</span>
            </button>
          ))}
        </div>
      )}
