...
/**
 * Employee Payroll History Checkpoint 1B — this report's own Print Options dialog, opened by its
 ...
 * table already shows (Step 11: the current paginated view, not an unbounded fetch).
 */
export function EmployeePayrollHistoryPrintOptionsDialog({
  open,
  onOpenChange,
  onConfirm,
}: {
  ...
  onConfirm: (selection: EmployeePayrollHistoryPrintSelection) => void;
}) {
  const [selection, setSelection] = useState<EmployeePayrollHistoryPrintSelection>(DEFAULT_PRINT_SELECTION);
