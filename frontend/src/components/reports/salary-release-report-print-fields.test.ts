...
} from './salary-release-report-print-fields';
 ...
 * than inferred from one rendered instance of the dialog. `correctionBalancePayable`/
 * `correctionBalanceRecovery(Total)` are deliberately NOT in this forbidden list — they are this
 * report's own frozen, approved fields (Original Released Amount disclosure), never a sensitive
 * exclusion.
 */
describe('print field vocabulary — sensitive-field exclusion (M2)', () => {
  ...
  it('the vocabulary keeps Original Released Amount and correction balances as distinct, separately-labeled fields', () => {
    const ids = TABLE_COLUMN_FIELDS.map((f) => f.id);
    expect(ids).toContain('originalReleasedAmount');
    expect(ids).toContain('correctionBalancePayable');
    expect(ids).toContain('correctionBalanceRecovery');
  });
});
...
describe('localStorage persistence', () => {
  ...
  it('uses its own versioned key, independent of the sibling reports’ own storage keys', () => {
    ...
    expect(window.localStorage.getItem('salary-release-report-print-fields:v1')).not.toBeNull();
    expect(window.localStorage.getItem('deduction-report-print-fields:v1')).toBeNull();
    expect(window.localStorage.getItem('overtime-report-print-fields:v1')).toBeNull();
  });

  it('silently drops unrecognized ids rather than crashing', () => {
    window.localStorage.setItem(
      'salary-release-report-print-fields:v1',
  ...
  it('returns null for corrupt JSON rather than throwing', () => {
    window.localStorage.setItem('salary-release-report-print-fields:v1', '{not json');
    expect(loadStoredPrintSelection()).toBeNull();
  });
});
