...
} from './overtime-report-print-fields';
...
describe('print field vocabulary — sensitive-field exclusion (M2)', () => {
  ...
  it('never offers Average OT Rate — a frozen, deliberately-not-implemented field', () => {
    const allIds = [...SUMMARY_CARD_FIELDS.map((f) => f.id), ...TABLE_COLUMN_FIELDS.map((f) => f.id)];
    for (const id of allIds) {
      expect(id.toLowerCase()).not.toContain('averageotrate');
    }
  });
});
...
describe('getReadabilityLevel', () => {
  ...
  it('returns very-wide once every column is selected (11-column max)', () => {
    expect(TABLE_COLUMN_FIELDS.length).toBe(11);
    expect(getReadabilityLevel(TABLE_COLUMN_FIELDS.length).status).toBe('very-wide');
  });
...
describe('localStorage persistence', () => {
  ...
  it('uses its own versioned key, independent of the sibling reports’ own storage keys', () => {
    ...
    expect(window.localStorage.getItem('overtime-report-print-fields:v1')).not.toBeNull();
  ...
  it('re-adds the locked column if a stored selection somehow lost it', () => {
    window.localStorage.setItem('overtime-report-print-fields:v1', JSON.stringify({ cards: [], columns: ['rowStatus'] }));
  ...
  it('returns null for corrupt JSON rather than throwing', () => {
    window.localStorage.setItem('overtime-report-print-fields:v1', '{not json');
    expect(loadStoredPrintSelection()).toBeNull();
  });
});
