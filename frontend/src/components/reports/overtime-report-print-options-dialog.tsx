...
/**
 * Overtime Report Checkpoint 1B — this report's own Print Options dialog, opened by its Print
 ...
 */
export function OvertimeReportPrintOptionsDialog({
  open,
  onOpenChange,
  onConfirm,
}: {
  ...
  onConfirm: (selection: OvertimeReportPrintSelection) => void;
}) {
  const [selection, setSelection] = useState<OvertimeReportPrintSelection>(DEFAULT_PRINT_SELECTION);
