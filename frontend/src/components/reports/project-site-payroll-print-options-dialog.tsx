...
} from './project-site-payroll-print-fields';

/**
 * Project Site Payroll Report Checkpoint 1B — this report's own Print Options dialog, opened by its
 * Print button before the browser's native print dialog (`useTriggerPrint`, the same shared print
 * engine every other page already calls — never a new one). Selection here is presentation-only:
 * every value a selected field renders comes from the exact same already-loaded page the on-screen
 * table already shows (Step 10: the current page only, not an unbounded fetch).
 */
export function ProjectSitePayrollPrintOptionsDialog({
  open,
  onOpenChange,
  onConfirm,
}: {
  ...
  onConfirm: (selection: ProjectSitePayrollPrintSelection) => void;
}) {
  const [selection, setSelection] = useState<ProjectSitePayrollPrintSelection>(DEFAULT_PRINT_SELECTION);
  ...
  // Reset restores every safe field (Step 10) — the same complete selection Select All produces,
  // since this dialog defines no narrower preset of its own.
