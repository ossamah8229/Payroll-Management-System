import type { DeductionReportRow, DeductionReportRowStatus } from '@payroll/shared';
import type { BadgeProps } from '@/components/ui/badge';

/**
 * Deduction Report Checkpoint 1B — pure label/tone lookups over the backend's own derived
 ...
 * `project-site-payroll-labels.ts`'s own tone mapping exactly, so the same status always reads the
 * same color across every report in this module.
 */

const ROW_STATUS_LABELS: Record<DeductionReportRowStatus, string> = {
  RELEASED: 'Released',
  HELD: 'Held',
...
export function rowStatusLabel(status: DeductionReportRowStatus): string {
...
export function rowStatusTone(status: DeductionReportRowStatus): NonNullable<BadgeProps['tone']> {
  switch (status) {
    case 'RELEASED':
      return 'green';
    case 'HELD':
...
export function primaryUnitLabel(row: Pick<DeductionReportRow, 'primaryUnit' | 'additionalUnitCount'>): string {
