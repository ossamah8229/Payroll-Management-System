...
describe('getReadabilityLevel', () => {
  ...
  it('returns very-wide once every column is selected (19-column max, scaled from Payroll Summary’s own thresholds)', () => {
    expect(getReadabilityLevel(TABLE_COLUMN_FIELDS.length).status).toBe('very-wide');
  });

  it('returns good at 9 columns and wide at 12, matching the documented thresholds', () => {
    expect(getReadabilityLevel(9).status).toBe('good');
    expect(getReadabilityLevel(12).status).toBe('wide');
  });
});
...
describe('localStorage persistence', () => {
  ...
  it('uses its own versioned key, independent of the sibling reports’ own storage keys', () => {
    ...
    expect(window.localStorage.getItem('project-site-payroll-print-fields:v1')).not.toBeNull();
    expect(window.localStorage.getItem('employee-payroll-history-print-fields:v1')).toBeNull();
    expect(window.localStorage.getItem('payroll-summary-print-fields:v1')).toBeNull();
  });
  ...
  it('returns null for corrupt JSON rather than throwing', () => {
    window.localStorage.setItem('project-site-payroll-print-fields:v1', '{not json');
    expect(loadStoredPrintSelection()).toBeNull();
  });
});
