...
/**
 * Variance / Month-on-Month Report Checkpoint 1B — this report's own Print Options dialog, opened
 * by its Print button before the browser's native print dialog (`useTriggerPrint`, the same shared
 * print engine every other page already calls — never a new one). Selection here is
 * presentation-only: every value a selected field renders comes from the exact same already-loaded
 * page the on-screen table already shows (current page only, not an unbounded fetch).
 */
export function VarianceReportPrintOptionsDialog({
  open,
  onOpenChange,
  onConfirm,
}: {
  ...
  onConfirm: (selection: VarianceReportPrintSelection) => void;
}) {
  const [selection, setSelection] = useState<VarianceReportPrintSelection>(DEFAULT_PRINT_SELECTION);
