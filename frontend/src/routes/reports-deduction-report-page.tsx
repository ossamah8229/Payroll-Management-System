...
import { DeductionReportPrintOptionsDialog } from '@/components/reports/deduction-report-print-options-dialog';
import {
  DEFAULT_PRINT_SELECTION,
  SUMMARY_CARD_FIELDS,
  TABLE_COLUMN_FIELDS,
  type DeductionReportPrintSelection,
  type SummaryCardFieldId,
  type TableColumnFieldId,
} from '@/components/reports/deduction-report-print-fields';
import { primaryUnitLabel, rowStatusLabel, rowStatusTone } from '@/components/reports/deduction-report-labels';
import {
  downloadDeductionReportExport,
  DeductionReportExportRowLimitExceededError,
  useDeductionReportList,
  DEDUCTION_REPORT_PAGE_SIZE,
  type DeductionReportFilters,
} from '@/hooks/use-deduction-report';
 ...
 * all five deduction tri-states (M2 print-content verification), never only a subset of them. */
function triStateSummaryLabel(state: TriState): string {
  return state === 'ALL' ? 'All' : state === 'YES' ? 'Yes' : 'No';
}

const ROW_STATUS_OPTIONS: DeductionReportRowStatus[] = ['RELEASED', 'HELD', 'NO_PAY_DUE', 'RECOVERY_DUE', 'PENDING'];
...
function summaryCardValue(id: SummaryCardFieldId, totals: ReportTotals): string {
  switch (id) {
    ...
    case 'employeesWithAnyDeduction':
      return totals.employeesWithAnyDeduction !== null ? String(totals.employeesWithAnyDeduction) : '—';
    case 'eobiTotal':
      return totals.eobiTotal !== null ? formatMoney(totals.eobiTotal) : '—';
    case 'advanceDeductionTotal':
      return totals.advanceDeductionTotal !== null ? formatMoney(totals.advanceDeductionTotal) : '—';
    case 'eidAdvanceDeductionTotal':
      return totals.eidAdvanceDeductionTotal !== null ? formatMoney(totals.eidAdvanceDeductionTotal) : '—';
    case 'fineTotal':
      return totals.fineTotal !== null ? formatMoney(totals.fineTotal) : '—';
    case 'correctionRecoveryTotal':
      return totals.correctionRecoveryTotal !== null ? formatMoney(totals.correctionRecoveryTotal) : '—';
...
function printColumnValue(id: TableColumnFieldId, row: DeductionReportRow): string {
  switch (id) {
    case 'employeeCode':
...
/**
 * Phase 7 Reports, Deduction Report Checkpoint 1B — the frontend for the frozen Checkpoint 1A
 * backend (`GET /api/v1/reports/deduction-report` and its export sibling,
 * `docs/architecture/workflows/reports.md` §17). Gated on `reports:view`, the same permission
 * Payroll Summary/Project Site Payroll Report already use — a single-cycle, site-scoped,
 * deduction-type-centric operational report, always scoped to exactly one required `PayrollCycle`
 * (no From/To range), never a cross-cycle, employee-searchable history.
 *
 * Every row is always the entry's own *original*, as-released figures via canonical `calcNet` — this
 * page never computes a financial value itself and never replays a correction; corrections are shown
 * only as a count column. Historical authorization is always `PayrollEntry.siteId`, never the
 * employee's current site. No detail page exists in V1 — every field this page needs lives directly
 * on the list row.
 */
export function ReportsDeductionReportPage({ user }: { user: SessionUser }) {
  ...
  const {
    ...
    error: cycleError,
    selectCycle,
  } = useSelectedPayrollCycle('reports/deduction-report');
  const hasAnyCycle = cycles.length > 0;

  const sites = useAccessibleProjectSites(user);
  const [selectedSiteIds, setSelectedSiteIds] = useState<string[]>([]);
  const [unitId, setUnitId] = useState('');
  const [rowStatus, setRowStatus] = useState<DeductionReportRowStatus | ''>('');
  const [hasCorrection, setHasCorrection] = useState<TriState>('ALL');
  const [hasEobi, setHasEobi] = useState<TriState>('ALL');
  const [hasAdvanceDeduction, setHasAdvanceDeduction] = useState<TriState>('ALL');
  const [hasEidAdvanceDeduction, setHasEidAdvanceDeduction] = useState<TriState>('ALL');
  const [hasFine, setHasFine] = useState<TriState>('ALL');
  const [hasCorrectionRecovery, setHasCorrectionRecovery] = useState<TriState>('ALL');
  const [sortBy, setSortBy] = useState<DeductionReportSortField>('employeeName');
  const [sortDir, setSortDir] = useState<DeductionReportSortDirection>('asc');
  const [page, setPage] = useState(1);
  const [activeExport, setActiveExport] = useState<ExportFormat | null>(null);
  const [printOptionsOpen, setPrintOptionsOpen] = useState(false);
  const [printSelection, setPrintSelection] = useState<DeductionReportPrintSelection>(DEFAULT_PRINT_SELECTION);
  ...
  useEffect(() => {
    ...
    hasEobi,
    hasAdvanceDeduction,
    hasEidAdvanceDeduction,
    hasFine,
    hasCorrectionRecovery,
    sortBy,
    sortDir,
  ]);

  const filters: DeductionReportFilters = {
    cycleId: cycleId ?? '',
    ...
    hasEobi: triStateToBoolean(hasEobi),
    hasAdvanceDeduction: triStateToBoolean(hasAdvanceDeduction),
    hasEidAdvanceDeduction: triStateToBoolean(hasEidAdvanceDeduction),
    hasFine: triStateToBoolean(hasFine),
    hasCorrectionRecovery: triStateToBoolean(hasCorrectionRecovery),
  };

  // No report request is ever made without a valid Cycle — `useDeductionReportList` is disabled
  // internally whenever `cycleId` is empty.
  const report = useDeductionReportList({
    ...filters,
    page,
    pageSize: DEDUCTION_REPORT_PAGE_SIZE,
    sortBy,
    sortDir,
  });
  ...
  function handleClearFilters() {
    ...
    setHasEobi('ALL');
    setHasAdvanceDeduction('ALL');
    setHasEidAdvanceDeduction('ALL');
    setHasFine('ALL');
    setHasCorrectionRecovery('ALL');
  }

  function handleSort(field: DeductionReportSortField) {
    if (field === sortBy) {
  ...
  async function handleExport(format: ExportFormat) {
    ...
    try {
      await downloadDeductionReportExport(cycle, filters, sortBy, sortDir, format);
    } catch (error) {
      if (error instanceof DeductionReportExportRowLimitExceededError) {
        toast.error(error.message);
      } else {
        toast.error(error instanceof ApiError ? error.message : `Deduction Report ${format.toUpperCase()} export failed`);
      }
    } finally {
  ...
  function handlePrintConfirm(selection: DeductionReportPrintSelection) {
    flushSync(() => {
  ...
  if (!canView) {
    return (
      <AppShell user={user} title="Deduction Report" subtitle="Operational deduction analysis for a single payroll cycle">
        <Card>
  ...
  const filtersActive = Boolean(
    selectedSiteIds.length ||
      ...
      hasCorrection !== 'ALL' ||
      hasEobi !== 'ALL' ||
      hasAdvanceDeduction !== 'ALL' ||
      hasEidAdvanceDeduction !== 'ALL' ||
      hasFine !== 'ALL' ||
      hasCorrectionRecovery !== 'ALL',
  );
  ...
  return (
    <AppShell user={user} title="Deduction Report" subtitle="Operational deduction analysis for a single payroll cycle">
            ...
            <PayrollPageToolbar
              ...
              filters={
                <>
                  ...
                  <FilterField id="dr-unit-filter" label={unitLabel}>
                    <select
                      id="dr-unit-filter"
                  ...
                  <FilterField id="dr-row-status" label="Row Status">
                    <select
                      id="dr-row-status"
                      className={selectClassName}
                      value={rowStatus}
                      onChange={(e) => setRowStatus(e.target.value as DeductionReportRowStatus | '')}
                      disabled={report.isFetching}
                    >
                  ...
                  <FilterField id="dr-has-correction" label="Has Correction">
                    <select
                      id="dr-has-correction"
                      className={selectClassName}
                      value={hasCorrection}
                  ...
                  <FilterField id="dr-has-eobi" label="Has EOBI">
                    <select
                      id="dr-has-eobi"
                      className={selectClassName}
                      value={hasEobi}
                      onChange={(e) => setHasEobi(e.target.value as TriState)}
                      disabled={report.isFetching}
                    >
                      <option value="ALL">All</option>
                      <option value="YES">Yes</option>
                  ...
                  <FilterField id="dr-has-advance-deduction" label="Has Advance Deduction">
                    <select
                      id="dr-has-advance-deduction"
                      className={selectClassName}
                      value={hasAdvanceDeduction}
                      onChange={(e) => setHasAdvanceDeduction(e.target.value as TriState)}
                      disabled={report.isFetching}
                    >
                      <option value="ALL">All</option>
                      <option value="YES">Yes</option>
                  ...
                  <FilterField id="dr-has-eid-advance-deduction" label="Has EID Advance Deduction">
                    <select
                      id="dr-has-eid-advance-deduction"
                      className={selectClassName}
                      value={hasEidAdvanceDeduction}
                      onChange={(e) => setHasEidAdvanceDeduction(e.target.value as TriState)}
                      disabled={report.isFetching}
                    >
                      <option value="ALL">All</option>
                      <option value="YES">Yes</option>
                  ...
                  <FilterField id="dr-has-fine" label="Has Fine">
                    <select
                      id="dr-has-fine"
                      className={selectClassName}
                      value={hasFine}
                      onChange={(e) => setHasFine(e.target.value as TriState)}
                      disabled={report.isFetching}
                    >
                      <option value="ALL">All</option>
                      <option value="YES">Yes</option>
                  ...
                  <FilterField id="dr-has-correction-recovery" label="Has Correction Recovery">
                    <select
                      id="dr-has-correction-recovery"
                      className={selectClassName}
                      value={hasCorrectionRecovery}
                      onChange={(e) => setHasCorrectionRecovery(e.target.value as TriState)}
                      disabled={report.isFetching}
                    >
                      <option value="ALL">All</option>
                      <option value="YES">Yes</option>
          ...
          <CardContent className="p-0">
              ...
              <div className="px-[18px] pt-[18px]">
                <PrintContextHeader
                  ...
                  context={[
                    ...
                    `Has EOBI: ${triStateSummaryLabel(hasEobi)}`,
                    `Has Advance Deduction: ${triStateSummaryLabel(hasAdvanceDeduction)}`,
                    `Has EID Advance Deduction: ${triStateSummaryLabel(hasEidAdvanceDeduction)}`,
                    `Has Fine: ${triStateSummaryLabel(hasFine)}`,
                    `Has Correction Recovery: ${triStateSummaryLabel(hasCorrectionRecovery)}`,
                    'Current page only',
                  ].join(' — ')}
                  showLogo
                />
            ...
            {!isLoading && !cycleError && !hasAnyCycle && (
              <div className="flex flex-col items-center gap-1 py-14 text-center">
                ...
                <p className="text-xs text-text-muted">A Deduction Report can only be generated once a payroll cycle exists.</p>
              </div>
            )}
            ...
            {!isLoading && !cycleError && hasAnyCycle && cycleId && report.error && (
              <div className="flex flex-col items-center gap-1 py-14 text-center">
                <p className="text-xs font-medium text-danger">Could not load the Deduction Report</p>
                  ...
                  <>
                    {totals && (
                      <div data-testid="on-screen-cards" className="flex flex-col gap-3 print:hidden">
                        <div>
                          <StatGroupLabel>Payroll Deductions</StatGroupLabel>
                          {totals.totalsComputed ? (
                            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                              <StatFigure label="EOBI" value={formatMoney(totals.eobiTotal ?? '0')} />
                              <StatFigure label="Advance Deduction" value={formatMoney(totals.advanceDeductionTotal ?? '0')} />
                              <StatFigure label="EID Advance Deduction" value={formatMoney(totals.eidAdvanceDeductionTotal ?? '0')} />
                              <StatFigure label="Fine" value={formatMoney(totals.fineTotal ?? '0')} />
                              <StatFigure label="Correction Recovery" value={formatMoney(totals.correctionRecoveryTotal ?? '0')} />
                              <StatFigure label="Total Deductions" value={formatMoney(totals.totalDeductions ?? '0')} testId="dr-stat-total-deductions" />
                            </div>
                          ) : (
                            <div className="flex flex-col justify-center rounded border border-border bg-surface px-3.5 py-3" data-testid="dr-totals-unavailable">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            <StatFigure label="Matching Entries" value={String(totals.matchingCount)} testId="dr-stat-matching-entries" />
                            <StatFigure
                              label="Employees With Any Deduction"
                              value={totals.employeesWithAnyDeduction !== null ? String(totals.employeesWithAnyDeduction) : '—'}
                              testId="dr-stat-employees-with-any-deduction"
                            />
                            <StatFigure label="Released" value={String(totals.releasedCount)} testId="dr-stat-released" />
                            <StatFigure label="Held" value={String(totals.heldCount)} testId="dr-stat-held" />
                            <StatFigure label="Pending" value={String(totals.pendingCount)} testId="dr-stat-pending" />
                            <StatFigure label="No Pay Due" value={String(totals.noPayDueCount)} testId="dr-stat-no-pay-due" />
                            <StatFigure label="Recovery Due" value={String(totals.recoveryDueCount)} testId="dr-stat-recovery-due" />
                            <StatFigure label="Corrected Entries" value={String(totals.correctedEntryCount)} testId="dr-stat-corrected-entries" />
                          </div>
                        </div>
                      </div>
                    )}
                      ...
                      <Table density="compact" className="min-w-full">
                        <TableHeader>
                          <TableRow>
                            ...
                            <SortableHead field="advanceDeduction" label="Advance Deduction" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} align="right" />
                            <SortableHead field="eidAdvanceDeduction" label="EID Advance Deduction" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} align="right" />
                            <SortableHead field="fine" label="Fine" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} align="right" />
                            <SortableHead field="correctionBalanceRecovery" label="Correction Bal. Recovery" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} align="right" />
                        ...
                        <TableBody>
                          {report.data.rows.map((row) => (
                            <TableRow key={row.payrollEntryId}>
                              ...
                              <TableCell className="whitespace-nowrap text-right font-semibold tabular-nums">{formatMoney(row.totalDeductions)}</TableCell>
      ...
      <DeductionReportPrintOptionsDialog open={printOptionsOpen} onOpenChange={setPrintOptionsOpen} onConfirm={handlePrintConfirm} />
    </AppShell>
  );
}
...
function SortableHead({
  ...
  field: DeductionReportSortField;
  label: string;
  sortBy: DeductionReportSortField;
  sortDir: DeductionReportSortDirection;
  onSort: (field: DeductionReportSortField) => void;
  align?: 'right';
}) {
